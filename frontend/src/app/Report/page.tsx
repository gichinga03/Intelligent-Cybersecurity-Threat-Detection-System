...
export default function Reports() {
  ...
  const summarizedLogs: Record<string, any> = logs.reduce((acc: any, log: any) => {
  ...
  // Apply filters
  const filteredLogs: any[] = Object.values(summarizedLogs).filter((log: any) => {
  ...
  return (
    ...
    className={`p-6 ${darkMode ? "bg-gray-900 text-white" : "bg-white text-black"}`}>
      <NavBar />
      <Sidebar isOpen={navSidebarOpen} setIsOpen={setNavSidebarOpen} />
